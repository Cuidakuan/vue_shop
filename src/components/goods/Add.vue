  ...
  methods: {
    ...
    async getCateList(){
      ...
      if(res.meta.status!==200){
        return this.$message.error('获取商品分类数据失败了');
      }
    ...
    beforeTabLeave(activeName,oldActiveName){
      console.log('即将离开的标签名称'+oldActiveName+'即将进入的标签名称'+activeName)
      if(oldActiveName==='0'&&this.addForm.goods_cat.length!==3)
      {
        this.$message.error('请先完善表格信息')
        return false
      }
   ...
   async tabClicked(){
      ...
      const {data:res} = await this.$http.get(`categories/${this.cateId}/attributes`,
        ...
        if(res.meta.status!==200){
          return this.$message.error('获取动态参数列表失败了')
        }
